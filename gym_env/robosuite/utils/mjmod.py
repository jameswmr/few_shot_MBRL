            ...
            for name in group_randomize_names:
                ...
                for attr, default_val in group_defaults[name].items():
                    ...
                    if settings["randomize"]:
                        ...
                        val_tmp = val + perturbation if settings["type"] == "size" else val * (1.0 + perturbation)
                        val = np.clip(val_tmp, *settings["clip"])
                        if 'stiffness' in attr:
                            val = 1000000000
                            #print("val", val, "default_value", default_val, "tmp_value", val_tmp, "name", name, "attr", attr)
                    # Modify this value
