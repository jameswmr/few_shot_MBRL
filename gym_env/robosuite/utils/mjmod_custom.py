...
class LightingModder(BaseModder):
    ...
    def __init__(
        ...
        randomize_pos_x=False,
        randomize_pos_y=False,
        randomize_pos_z=False,
        randomize_direction=False,
        # randomize_dir_x=False,
        # randomize_dir_y=False,
        # randomize_dir_z=False,
        randomize_specular=False,
        # randomize_ambient=False,
        randomize_ambient_r=False,
        randomize_ambient_g=False,
        randomize_ambient_b=False,
        randomize_diffuse=False,
        randomize_active=False,
        pos_x_perturbation_size=0.1,
        pos_y_perturbation_size=0.1,
        pos_z_perturbation_size=0.1,
        # position_perturbation_size=0.1,
        direction_perturbation_size=0.35,  # 20 degrees
        # dir_x_perturbation_size=0.35,
        # dir_y_perturbation_size=0.35,
        # dir_z_perturbation_size=0.35,
        specular_perturbation_size=0.1,
        ambient_r_perturbation_size=0.1,
        ambient_g_perturbation_size=0.1,
        ambient_b_perturbation_size=0.1,
        ...
        self.randomize_pos_x = randomize_pos_x
        self.randomize_pos_y = randomize_pos_y
        self.randomize_pos_z = randomize_pos_z
        self.randomize_direction = randomize_direction
        # self.randomize_dir_x = randomize_dir_x
        # self.randomize_dir_y = randomize_dir_y
        # self.randomize_dir_z = randomize_dir_z
        self.randomize_specular = randomize_specular
        # self.randomize_ambient = randomize_ambient
        self.randomize_ambient_r = randomize_ambient_r
        self.randomize_ambient_g = randomize_ambient_g
        self.randomize_ambient_b = randomize_ambient_b
        ...
        self.pos_x_perturbation_size = pos_x_perturbation_size
        self.pos_y_perturbation_size = pos_y_perturbation_size
        self.pos_z_perturbation_size = pos_z_perturbation_size
        self.direction_perturbation_size = direction_perturbation_size
        # self.dir_x_perturbation_size = dir_x_perturbation_size
        # self.dir_y_perturbation_size = dir_y_perturbation_size
        # self.dir_z_perturbation_size = dir_z_perturbation_size
        self.specular_perturbation_size = specular_perturbation_size
        # self.ambient_perturbation_size = ambient_perturbation_size
        self.ambient_r_perturbation_size = ambient_r_perturbation_size
        self.ambient_g_perturbation_size = ambient_g_perturbation_size
        self.ambient_b_perturbation_size = ambient_b_perturbation_size
    ...
    def get_context(self):
        ...
        for name in self.light_names:
            ...
            context[f"{header}@{light_name}@specular"] = np.array(
                self.get_specular(name)
            )
            # context[f"{header}@{light_name}@ambient"] = np.array(self.get_ambient(name))
            context[f"{header}@{light_name}@ambient_r"] = np.array(
                self.get_ambient(name)[0]
            )
            context[f"{header}@{light_name}@ambient_g"] = np.array(
                self.get_ambient(name)[1]
            )
            context[f"{header}@{light_name}@ambient_b"] = np.array(
                self.get_ambient(name)[2]
            )
            context[f"{header}@{light_name}@diffuse"] = np.array(self.get_diffuse(name))
            context[f"{header}@{light_name}@active"] = self.get_active(name)
        return context
    ...
    def set_context(self, context):
        for key, val in context.items():
            ...
            if header == "lighting":
                ...
                if attr == "pos_x":
                    self.set_pos_x(light_name, val)
                elif attr == "pos_y":
                    self.set_pos_y(light_name, val)
                elif attr == "pos_z":
                ...
                #     self.set_dir_z(light_name, val)
                    ...
                    self.set_ambient_r(light_name, val)
                elif attr == "ambient_g":
                    self.set_ambient_g(light_name, val)
                elif attr == "ambient_b":
                    self.set_ambient_b(light_name, val)
                elif attr == "diffuse":
                    self.set_diffuse(light_name, val)
                elif attr == "active":
                    self.set_active(light_name, val)
                else:
                    raise ValueError(
                        f"Wrong attribute for the lighting modder: '{attr}'"
                    )
    ...
    def randomize(self):
        ...
        """
        for name in self.light_names:
            if self.randomize_pos_x:
                self._randomize_pos_x(name)

            if self.randomize_pos_y:
                self._randomize_pos_y(name)
            ...
            if self.randomize_ambient_g:
                self._randomize_ambient_g(name)

            if self.randomize_ambient_b:
                self._randomize_ambient_b(name)
    ...
    def _randomize_pos_x(self, name):

        delta_pos_x = self.random_state.uniform(
            low=-self.pos_x_perturbation_size,
            high=self.pos_x_perturbation_size,
            size=1,
        )
        self.set_pos_x(
            name,
            self._defaults[name]["pos"][0] + delta_pos_x,
        )

    def _randomize_pos_y(self, name):
        delta_pos_y = self.random_state.uniform(
            low=-self.pos_y_perturbation_size,
            high=self.pos_y_perturbation_size,
            size=1,
        )
        self.set_pos_y(
            name,
            self._defaults[name]["pos"][1] + delta_pos_y,
        )

    def _randomize_pos_z(self, name):
        delta_pos_z = self.random_state.uniform(
            low=-self.pos_z_perturbation_size,
            high=self.pos_z_perturbation_size,
            size=1,
        )
        self.set_pos_z(
            name,
            self._defaults[name]["pos"][2] + delta_pos_z,
        )
    ...
    def _randomize_ambient_r(self, name):
        delta = self.random_state.uniform(
            low=-self.ambient_r_perturbation_size,
            high=self.ambient_r_perturbation_size,
            size=1,
        )
        self.set_ambient_r(
            name,
            self._defaults[name]["ambient"][0] + delta,
        )

    def _randomize_ambient_g(self, name):
        delta = self.random_state.uniform(
            low=-self.ambient_g_perturbation_size,
            high=self.ambient_g_perturbation_size,
            size=1,
        )
        self.set_ambient_g(
            name,
            self._defaults[name]["ambient"][1] + delta,
        )

    def _randomize_ambient_b(self, name):
        delta = self.random_state.uniform(
            low=-self.ambient_b_perturbation_size,
            high=self.ambient_b_perturbation_size,
            size=1,
        )
        self.set_ambient_b(
            name,
            self._defaults[name]["ambient"][2] + delta,
        )
    ...
    def set_pos_x(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_pos[lightid][0] = value

    def set_pos_y(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_pos[lightid][1] = value

    def set_pos_z(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_pos[lightid][2] = value
    ...
    # def set_dir_x(self, name, value):
    #     lightid = self.get_lightid(name)
    #     assert lightid > -1, "Unkwnown light %s" % name

    #     self.model.light_dir[lightid][0] = value

    # def set_dir_y(self, name, value):
    #     lightid = self.get_lightid(name)
    #     assert lightid > -1, "Unkwnown light %s" % name

    #     self.model.light_dir[lightid][1] = value

    # def set_dir_z(self, name, value):
    #     lightid = self.get_lightid(name)
    #     assert lightid > -1, "Unkwnown light %s" % name

    #     self.model.light_dir[lightid][2] = value
    ...
    def set_ambient_r(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_ambient[lightid][0] = value

    def set_ambient_g(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_ambient[lightid][1] = value

    def set_ambient_b(self, name, value):
        lightid = self.get_lightid(name)
        assert lightid > -1, "Unkwnown light %s" % name

        self.model.light_ambient[lightid][2] = value
...
class TextureModder(BaseModder):
    ...
    def save_defaults(self):
        ...
        for name in self.geom_names:
            if self._check_geom_for_texture(name):
                ...
                self._defaults[name]["material_reflectance"] = (
                    self.get_material_reflectance(name)
                )
                self._defaults[name]["material_shininess"] = (
                    self.get_material_shininess(name)
                )
                self._defaults[name]["material_specular"] = self.get_material_specular(
                    name
                )
            else:
                # store geom color
                self._defaults[name]["r"] = np.array(self.get_geom_r(name))
                self._defaults[name]["g"] = np.array(self.get_geom_g(name))
                self._defaults[name]["b"] = np.array(self.get_geom_b(name))
    ...
    def get_context(self):
        ...
        for name in self.geom_names:
            if name is None:
                name = "texture"
            context[f"{header}@{name}@geom_r"] = np.array(self.get_geom_r(name))
            context[f"{header}@{name}@geom_g"] = np.array(self.get_geom_g(name))
            context[f"{header}@{name}@geom_b"] = np.array(self.get_geom_b(name))

            if self._check_geom_for_texture(name):
                # context[f"{header}@{name}@texture"] = np.array(self.get_texture(name))
                context[f"{header}@{name}@material_reflectance"] = np.array(
                    self.get_material_reflectance(name)
                )
                context[f"{header}@{name}@material_shininess"] = np.array(
                    self.get_material_shininess(name)
                )
                context[f"{header}@{name}@material_specular"] = np.array(
                    self.get_material_specular(name)
                )
                # context[f"{header}@{name}@checker_matrices"] = np.array(self.get_checker_matrices(name))
        return context
    ...
    def set_context(self, context):
        for key, val in context.items():
            ...
            if header == "texture":
                ...
                if attr == "geom_r":
                    self.set_geom_r(texture_name, val)
                elif attr == "geom_g":
                    self.set_geom_g(texture_name, val)
                elif attr == "geom_b":
                    self.set_geom_b(texture_name, val)
                elif attr == "material_reflectance":
                    self.set_material_reflectance(texture_name, val)
                elif attr == "material_shininess":
                    self.set_material_shininess(texture_name, val)
                elif attr == "material_specular":
                    self.set_material_specular(texture_name, val)
                else:
                    raise ValueError(
                        f"Wrong attribute for the lighting modder: '{attr}'"
                    )
    ...
    def get_geom_r(self, name):
        geom_id = self.model.geom_name2id(name)
        return self.model.geom_rgba[geom_id, 0]

    def get_geom_g(self, name):
        geom_id = self.model.geom_name2id(name)
        return self.model.geom_rgba[geom_id, 1]

    def get_geom_b(self, name):
        geom_id = self.model.geom_name2id(name)
        return self.model.geom_rgba[geom_id, 2]
    ...
    def set_geom_r(self, name, value):
        geom_id = self.model.geom_name2id(name)
        self.model.geom_rgba[geom_id, 0] = value

    def set_geom_g(self, name, value):
        geom_id = self.model.geom_name2id(name)
        self.model.geom_rgba[geom_id, 1] = value

    def set_geom_b(self, name, value):
        geom_id = self.model.geom_name2id(name)
        self.model.geom_rgba[geom_id, 2] = value
    ...
    def get_material_reflectance(self, name):
        mat_id = self._name_to_mat_id(name)
        material_reflectance = np.array(self.model.mat_reflectance[mat_id])

        return material_reflectance

    def get_material_shininess(self, name):
        mat_id = self._name_to_mat_id(name)
        material_shininess = np.array(self.model.mat_shininess[mat_id])

        return material_shininess

    def get_material_specular(self, name):
        mat_id = self._name_to_mat_id(name)
        material_specular = np.array(self.model.mat_specular[mat_id])
    ...
    def set_material_reflectance(self, name, value):
        mat_id = self._name_to_mat_id(name)
        self.model.mat_reflectance[mat_id] = value

    def set_material_shininess(self, name, value):
        mat_id = self._name_to_mat_id(name)
        self.model.mat_shininess[mat_id] = value

    def set_material_specular(self, name, value):
        mat_id = self._name_to_mat_id(name)
...
class DynamicsModder(BaseModder):
    ...
    def __init__(
        ...
        randomize_inertia_ixx=False,
        randomize_inertia_iyy=False,
        randomize_inertia_izz=False,
        randomize_mass=False,
        position_perturbation_size=0.02,
        quaternion_perturbation_size=0.02,
        # inertia_perturbation_ratio=0.02,
        inertia_perturbation_ixx_ratio=0.02,
        inertia_perturbation_iyy_ratio=0.02,
        inertia_perturbation_izz_ratio=0.02,
        mass_perturbation_ratio=0.02,
        # Geom parameters
        geom_names=None,
        # randomize_friction=False,
        randomize_friction_sliding=False,
        randomize_friction_torsional=False,
        randomize_friction_rolling=False,
        randomize_time_constant=False,
        randomize_damping_ratio=False,
        # randomize_solref=False,
        randomize_solimp=False,
        friction_sliding_perturbation_ratio=0.1,
        friction_torsional_perturbation_ratio=0.1,
        friction_rolling_perturbation_ratio=0.1,
        # solref_perturbation_ratio=0.1,
        time_constant_perturbation_ratio=0.1,
        damping_ratio_perturbation_ratio=0.1,
        ...
        self.body_randomizations = {
            ...
            # },
            "inertia_ixx": {
                "randomize": randomize_inertia_ixx,
                "perturbation": inertia_perturbation_ixx_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "inertia_iyy": {
                "randomize": randomize_inertia_iyy,
                "perturbation": inertia_perturbation_iyy_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "inertia_izz": {
                "randomize": randomize_inertia_izz,
                "perturbation": inertia_perturbation_izz_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "mass": {
        ...
        self.geom_randomizations = {
            "friction_sliding": {
                "randomize": randomize_friction_sliding,  # Bool
                "perturbation": friction_sliding_perturbation_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "friction_torsional": {
                "randomize": randomize_friction_torsional,  # Bool
                "perturbation": friction_torsional_perturbation_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "friction_rolling": {
                "randomize": randomize_friction_rolling,  # Bool
                "perturbation": friction_rolling_perturbation_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "time_constant": {
                "randomize": randomize_time_constant,
                "perturbation": time_constant_perturbation_ratio,
                "type": "ratio",
                "clip": (0.0, np.inf),
            },
            "damping_ratio": {
                "randomize": randomize_damping_ratio,
                "pertubation": damping_ratio_perturbation_ratio,
                "type": "ratio",
                "clip": (0.6, 1.35),  # (0.5*1.2 ~ 1.1*1.2)
            },
            # "solref": {
    ...
    def save_defaults(self):
        ...
        for body_name in self.sim.model.body_names:
            ...
            self.body_defaults[body_name] = {
                ...
                "euler": trans.quat2euler(self.sim.model.body_quat[body_id]),
                # "inertia": np.array(self.sim.model.body_inertia[body_id]),
                "inertia_ixx": self.sim.model.body_inertia[body_id][0],
                "inertia_iyy": self.sim.model.body_inertia[body_id][1],
                "inertia_izz": self.sim.model.body_inertia[body_id][2],
        ...
        for geom_name in self.sim.model.geom_names:
            ...
            self.geom_defaults[geom_name] = {
                "friction_sliding": np.array(self.sim.model.geom_friction[geom_id][0]),
                "friction_torsional": np.array(
                    self.sim.model.geom_friction[geom_id][1]
                ),
                "friction_rolling": np.array(self.sim.model.geom_friction[geom_id][2]),
                "time_constant": np.array(self.sim.model.geom_solref[geom_id][0]),
                "damping_ratio": np.array(self.sim.model.geom_solref[geom_id][1]),
    ...
    def get_context(self):
        """
        get the context from the env
        """

        header = "dynamic"
        context = {
            f"{header}@env@density": self.sim.model.opt.density,
            f"{header}@env@viscosity": self.sim.model.opt.viscosity,
        }

        for body_name in self.sim.model.body_names:
            body_id = self.sim.model.body_name2id(body_name)
            context[f"{header}@{body_name}@position"] = np.array(
                self.sim.model.body_pos[body_id]
            )
            context[f"{header}@{body_name}@quaternion"] = np.array(
                self.sim.model.body_quat[body_id]
            )
            # print (context[f"{header}@{body_name}@quaternion"],"check")
            context[f"{header}@{body_name}@euler"] = trans.quat2euler(
                self.sim.model.body_quat[body_id]
            )
            # context[f"{header}@{body_name}@inertia"] = np.array(self.sim.model.body_inertia[body_id])
            context[f"{header}@{body_name}@inertia_ixx"] = self.sim.model.body_inertia[
                body_id
            ][0]
            context[f"{header}@{body_name}@inertia_iyy"] = self.sim.model.body_inertia[
                body_id
            ][1]
            context[f"{header}@{body_name}@inertia_izz"] = self.sim.model.body_inertia[
                body_id
            ][2]
            context[f"{header}@{body_name}@mass"] = self.sim.model.body_mass[body_id]

        for geom_name in self.sim.model.geom_names:
            geom_id = self.sim.model.geom_name2id(geom_name)
            context[f"{header}@{geom_name}@friction_sliding"] = np.array(
                self.sim.model.geom_friction[geom_id][0]
            )
            context[f"{header}@{geom_name}@friction_torsional"] = np.array(
                self.sim.model.geom_friction[geom_id][1]
            )
            context[f"{header}@{geom_name}@friction_rolling"] = np.array(
                self.sim.model.geom_friction[geom_id][2]
            )
            context[f"{header}@{geom_name}@time_constant"] = np.array(
                self.sim.model.geom_solref[geom_id][0]
            )
            context[f"{header}@{geom_name}@damping_ratio"] = np.array(
                self.sim.model.geom_solref[geom_id][1]
            )
            # context[f"{header}@{geom_name}@solref"] = np.array(self.sim.model.geom_solref[geom_id])
            context[f"{header}@{geom_name}@solimp"] = np.array(
                self.sim.model.geom_solimp[geom_id]
            )
        ...
        for joint_name in self.sim.model.joint_names:
            ...
            context[f"{header}@{joint_name}@stiffness"] = self.sim.model.jnt_stiffness[
                joint_id
            ]
            context[f"{header}@{joint_name}@frictionloss"] = np.array(
                self.sim.model.dof_frictionloss[dof_idx]
            )
            context[f"{header}@{joint_name}@damping"] = np.array(
                self.sim.model.dof_damping[dof_idx]
            )
            context[f"{header}@{joint_name}@armature"] = np.array(
                self.sim.model.dof_armature[dof_idx]
            )
    ...
    def set_context(self, context):
        for key, val in context.items():
            ...
            if header == "dynamic":
                if name == "env":
                    name = None
                if name is None and attr == "integrator":
                    self.sim.model.opt.integrator = val
                else:
                    self.mod(name=name, attr=attr, val=val)
        self.update()
    ...
    def mod_quaternion(self, name, val=(0, 1, 0, 0)):
        """
    ...
    def mod_euler(self, name, val=(0, 0, 0)):
        body_id = self.sim.model.body_name2id(name)
        quat = trans.euler2quat(val[0], val[1], val[2])
        quat = np.array(quat) / np.linalg.norm(quat)
        self.sim.model.body_quat[body_id] = quat  # It needs to be in quat form
    ...
    def mod_inertia_ixx(self, name, val):

        if name not in self.dummy_bodies:
            body_id = self.sim.model.body_name2id(name)
            self.sim.model.body_inertia[body_id][0] = np.array(val)

    def mod_inertia_iyy(self, name, val):

        if name not in self.dummy_bodies:
            body_id = self.sim.model.body_name2id(name)
            self.sim.model.body_inertia[body_id][1] = np.array(val)

    def mod_inertia_izz(self, name, val):

        if name not in self.dummy_bodies:
            body_id = self.sim.model.body_name2id(name)
            self.sim.model.body_inertia[body_id][2] = np.array(val)
    ...
    def mod_friction_sliding(self, name, val):
        """
        ...
        self.sim.model.geom_friction[geom_id][0] = np.array(val)

    def mod_friction_torsional(self, name, val):
        """
        Modifies the @name's geom friction within the simulation.
        See http://www.mujoco.org/book/XMLreference.html#geom for more details.
        ...
        # Modify this value
        geom_id = self.sim.model.geom_name2id(name)
        self.sim.model.geom_friction[geom_id][1] = np.array(val)

    def mod_friction_rolling(self, name, val):
        """
        Modifies the @name's geom friction within the simulation.
        See http://www.mujoco.org/book/XMLreference.html#geom for more details.
        ...
        # Modify this value
        geom_id = self.sim.model.geom_name2id(name)
        self.sim.model.geom_friction[geom_id][2] = np.array(val)

    def mod_time_constant(self, name, val):
        """
        Modifies the @name's geom contact solver parameters within the simulation.
        See http://www.mujoco.org/book/modeling.html
        """
        # Modify this value
        geom_id = self.sim.model.geom_name2id(name)
        self.sim.model.geom_solref[geom_id][0] = np.array(val)

    def mod_damping_ratio(self, name, val):
        """
        Modifies the @name's geom contact solver parameters within the simulation.
        See http://www.mujoco.org/book/modeling.html
        """
        ...
        self.sim.model.geom_solref[geom_id][1] = np.array(val)
        # print ("Moding damping ratio to : ", np.array(val))
