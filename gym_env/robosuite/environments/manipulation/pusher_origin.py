...
from robosuite.models.objects import X_Right_Wall_Object, X_Left_Wall_Object, Y_Front_Wall_Object, Y_Back_Wall_Object, Block
...
class Pusher(SingleArmEnv):
    ...
    def __init__(
            ...
            random_target_generation=False,  # Below Added by Xilun
            threshold=0.1,
    ...
    def reward(self, action):
        ...
        if self.reward_shaping:
            ...
            """ sliding reward -- friction -- origin from Push_v4.py in Gym"""
            # self.get_body_com("object") = self.sim.data.body_xpos[self.plate1_body_id]
            plate1_pos = self.sim.data.body_xpos[self.plate1_body_id]
            plate2_pos = self.sim.data.body_xpos[self.plate2_body_id]
            gripper_site_pos = self.sim.data.site_xpos[self.robots[0].eef_site_id]

            vec_1 = plate1_pos - gripper_site_pos  # Distance between plate1 and gripper pos
            vec_2 = plate2_pos - self.target_pos  # Distance between cube and target pos
            ...
            # reward_nd = reward_dist + 0.5 * reward_near * 0.0 + 0.1 * action_reward 
            reward_nd = reward_dist + 0.3 * reward_near + 0.1 * action_reward # Try Try 
            reward += reward_nd
        else:
    ...
    def _load_model(self):
        ...
        # if self.cube_init_position is None:
        #     x_range = np.asarray([-0.4, -0.3])
        #     y_range = np.asarray([-0.1, 0.1])
        # else:
        #     x_pose = self.cube_init_position[0]
        #     y_pose = self.cube_init_position[1]
        #     x_range = np.asarray([x_pose, x_pose])
        #     y_range = np.asarray([y_pose, y_pose])
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.knob, self.plate1,
    ...
    def reset(self):
        ...
        # if self.dynamics_modder is None:
        #     self.dynamics_modder = DynamicsModder(sim=self.sim)
        # if self.lighting_modder is None:
        #     self.lighting_modder = LightingModder(sim=self.sim)
