...
if __name__ == "__main__":
    ...
    def print_params():
        ...
        print("Density:", env.sim.model.opt.density)
        print("Viscosity:", env.sim.model.opt.viscosity)
        # cube_body_id = env.sim.model.body_name2id(env.plate.root_body)
        cube_geom_ids = [env.sim.model.geom_name2id(geom) for geom in env.plate.contact_geoms]
        print(f"cube frictions: {env.sim.model.geom_friction[cube_geom_ids]}")
        # print(f"cube mass: {env.sim.model.body_mass[cube_body_id]}")
        stiff_joint_ids = [env.sim.model.joint_name2id(joint_name) for joint_name in env.sim.model.joint_names]
        print(f"Joint Stiffness: {env.sim.model.jnt_stiffness[stiff_joint_ids]}")
